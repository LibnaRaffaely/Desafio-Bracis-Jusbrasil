...
Branch `modulo4-decisao`. Entregáveis: `src/citacoes/nos/decidir.py`
(+ export em `nos/__init__.py`), `tests/test_nos_decidir.py`,
`scripts/avalia_decisao.py`, este relatório.

Ambiente de medição: Python 3.14 do sistema (sem `uv`/`.venv` nesta
máquina), venv temporário fora do repo com `unidecode pytest hypothesis
ruff`; `PYTHONPATH=src`. Dados em pasta externa passada por `--dados`; base
aberta em `?mode=ro`. Catálogo real cacheado em `artifacts/catalogo_canonico.json`
e CSVs por citação em `outputs/` — os dois gitignored, nada entrou no `git status`.
...
`buscar_no_catalogo`); com modelo, injeta `JuizLLM`, que hoje levanta
`NotImplementedError` — ligar o juiz falha alto em vez de silenciar.
